...
const logger = {
	...
	warn: (message: string, additionalInfo?: any) => {
		console.warn(
			`[${_MESSAGE_PREFIX}] \x1B[${consoleConfig.colours.yellow}mWARN:\x1b[0m ${message} ${additionalInfo ?? ''}`,
		);
	},
	error: (message: string, additionalInfo?: any) => {
		console.error(
			...
			};${consoleConfig.modifiers.underline}mERROR:\x1b[0m ${message} ${additionalInfo ?? ''}`,
		);
	},
};
